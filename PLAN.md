...
- `query_radius(x, radius, *, p=2.0, return_distance=False, sort=False, parallel=None)`
- `query_pairs(radius, *, p=2.0)`
- `__len__()`
- `__repr__()`
...
- `query_radius`
  - 1 点と batch の両方を受ける
  - `return_distance=False` なら index list を返す
  - `return_distance=True` なら `(indices, distances)` を返す
- `query_pairs`
  - `radius` 以内のペアを列挙する
  - Python では `list[tuple[int, int]]` を返す
...
- leaf node は `start..end` の範囲だけを持つ
...
- branch-and-bound
- `k == 1` は specialized path を用意する
- `k > 1` は bounded max-heap
- 下界評価には node bounding box を使う
- query stack は iterative に実装して再帰を避ける

### radius アルゴリズム

- bounding box 下界で枝刈り
- leaf では brute-force
- batch radius query では result buffer を point ごとに持つ

### pair enumeration

- 同一 tree 内 pair 探索を専用実装する
- node-node traversal で枝刈りする
- naive O(n^2) fallback は leaf 同士に限定する
...
- `query_radius` は結果長が不揃いなので Python list ベースを使う
...
- radius query correctness
- pair enumeration correctness
- duplicate points
- degenerate data

### Rust property tests

- 小規模ランダム点群で brute-force と一致するかを確認する
- candidate:
  - `proptest`
...
- radius query
- query_pairs
- free-threaded Python 上での multi-thread query safety
...
- `eps`
- `max_distance`
- specialized path と generic path の整理
...
- `query_radius` の可変長返り値は Python 側 boxing コストが残る
...
1. `pyproject.toml` を free-threaded 配布方針に更新する
2. `Cargo.toml` を `PyO3 0.28.3` / `numpy 0.28.0` / `rayon` 前提に更新する
3. `python/kdtree/` package と `_core.pyi` を作る
4. Rust core の data structure と build を実装する
